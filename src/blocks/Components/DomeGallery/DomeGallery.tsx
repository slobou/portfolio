...
function buildItems(pool: MediaItem[], seg: number): ItemDef[] {
  ...
  const normalizedMedia = pool.map((media) => {
    if (typeof media === "string") {
      // Auto-detect type based on file extension
      const isVideo = /\.(mp4|webm|ogg|mov|avi|mkv)$/i.test(media);
      return {
        src: media,
        alt: "",
    ...
    return {
      ...
      type:
        media.type ||
        (() => {
          // Auto-detect type based on file extension if not specified
    ...
    if (usedMedia[i].src === usedMedia[i - 1].src) {
      for (let j = i + 1; j < usedMedia.length; j++) {
        if (usedMedia[j].src !== usedMedia[i].src) {
          const tmp = usedMedia[i];
          usedMedia[i] = usedMedia[j];
          usedMedia[j] = tmp;
          break;
        }
      }
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    // Preload images using Image objects (forces browser to cache them)
    ...
    // Also add link preload tags for critical images (first 15 for better coverage)
    const imageArray = Array.from(uniqueImages).slice(0, 15);
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (originalMedia) {
        // If it's a video, pause and mute it before cloning
        if (originalMedia instanceof HTMLVideoElement) {
          originalMedia.pause();
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    if (mediaType === "video") {
      const video = document.createElement("video");
      video.src = rawSrc;
      video.setAttribute("aria-label", rawAlt);
    ...
    // Calculate center position for final size
    const finalWidthNum = parseFloat(finalWidth);
    const finalHeightNum = parseFloat(finalHeight);
    const centerOffsetX = (frameR.width - finalWidthNum) / 2;
    const centerOffsetY = (frameR.height - finalHeightNum) / 2;

    requestAnimationFrame(() => {
      overlay.style.opacity = "1";
      overlay.style.transform = `translate(${centerOffsetX}px, ${centerOffsetY}px) scale(1, 1)`;
              ...
              {items.map((it, i) => (
                ...
                >
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    aria-label={it.alt || `Open ${it.type}`}
                    onClick={(e) => {
